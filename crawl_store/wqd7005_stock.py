...
for item in sector_key_value:
    for k, v in item.items():
        stock_list.append(["https://s3-ap-southeast-1.amazonaws.com/biz.thestar.com.my/json/sectors/{0}/{1}/stocks.js".format(k, v), k, v])
...
def crawl_data(name, sector, subsector):
    ...
    data = eval(body)
    data_transfer(data, name["counter"], name["stockcode"], sector, subsector)

def data_transfer(data, company_name, stock_code, sector, subsector):
    try:
        data_len = len(data["t"])
